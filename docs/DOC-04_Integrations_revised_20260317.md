...
To preserve “open-core” flexibility, AdvisoryOps should be framed as an **open intelligence layer / commercial action layer** product.

### Core (safe to open-source)
- Data contracts & schemas (DOC-02)
- Mitigation playbook (DOC-03)
- Public normalized advisory corpus derived from public sources
- Correlated issue IDs / issue-cluster outputs for public advisories
- Basic remediation visibility (vendor fix / workaround / field notice links + normalized remediation status)
...
- Customer-specific applicability matching and explainability workflows
- Automated risk scoring using local environment context
- Vendor-specific ITSM connectors (beyond any baseline connector shipped in Core)
- Asset enrichment connectors requiring customer credentials
- UI features, deployment hardening, multi-tenant features
- Task routing, approvals, alerting, dashboards, and audit workflows
- Customer-specific workflow rules and mapping presets

**Note:** A useful rule of thumb is:
- **Core** answers: “What happened, what is affected in general, and what fixes/workarounds are publicly known?”
- **Pro** answers: “What does this mean for this customer right now, what should they prioritize, and how is the work executed and tracked?”

**ServiceNow note:** ServiceNow can still be either:
- in **Core** (maximize adoption), or
- in **Pro** (monetize the “last-mile ticketing”)
Build the interface in Core regardless.
